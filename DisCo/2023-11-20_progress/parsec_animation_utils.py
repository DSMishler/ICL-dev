...
def guess_trace_version(trace):
    for key in trace:
        # print(key)
        if "/event_infos_" == key[:13]:
            return "v2"
    return "v1"
...
def animate_trace(tracefiles, 
    ...
    if(type(tracefiles) is list):
        print("Passed multiple tracefiles.") #Concatenating them.")
        print("not implemented. You'll have to implement this one, Danny")
        # TODO: implement
    ...
    trace_version = guess_trace_version(trace)
    if trace_version != "v2":
        print(f"we only do trace version 2 here. Yours is {trace_version}.")
        return
    ...
    ## TODO: allow multiple work tasks
    if len(work_tasks_indices) != 1:
        print("this software is too scrambled to handle that right now. Go fix it, Danny.")
        return
    work_task_index = work_tasks_indices[0]
    tevent_name="event_infos_"+str(work_task_index)
    """
    ...
    if trace_type == "dtd":
        ...
        orderdf = pd.DataFrame(trace.events[work_tasks])
    """
    mdf = pd.DataFrame(trace.events)
    tdf = pd.DataFrame(trace[tevent_name])
    orderdf = mdf.join(tdf,how="inner")
    ...
    if(expected_tasks != len(orderdf)):
        ...
        if(len(orderdf) > expected_tasks):
            print(f"Because {len(orderdf)} > {expected_tasks}, I will assume you "
                  + f"included a warmup and I will consider the last {expected_tasks} tasks")
            if(trace_type == "ptg"):
    ...
    def animate_gemm_common(frame, trace_type, plots, mode):
        ...
        for index, task in tasks_during.iterrows():
            ...
            if k is None:
                print("hollup, why is k None?")
                continue
                k = 0
            if m is None:
                print("hollup, why is m None?")
                continue
                m = 0
            if n is None:
                print("hollup, why is n None?")
                continue
                n = 0
            ...
            ### Sanity check from 2023-09-25
            # if m % 6 == 0 and n % 6 == 0:
                # print(f"({m},{n},{k}) by core {core}")
